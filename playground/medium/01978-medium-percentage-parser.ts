...
//   A extends `${infer Head extends Prefix}${infer Num}%`
//   ? [Head, Num, "%"]
//   : A extends `${infer Head extends Prefix}${infer Num}`
//     ? [Head, Num, ""]
//     : A extends `${infer Num}%`
...
type ParseNumber<T extends string> = T extends `${ParseSign<T>}${infer Num}${ParsePercent<T>}` ? Num : "";
...
type B = PercentageParser<'+'>;
  // ^?
type C = PercentageParser<'+1'>;
  // ^?
type D = PercentageParser<'+100'>;
  // ^?
type E = PercentageParser<'+100%'>;
  // ^?
type F = PercentageParser<'-100'>;
  // ^?
